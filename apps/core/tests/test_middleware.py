...
class TestObservabilidadeMiddleware:
    ...
    def test_propaga_correlation_id_do_request(self, db):
        ...
        client = APIClient()
        resp = client.get(
            "/api/v1/institucional/health/live/", HTTP_X_CORRELATION_ID="meu-id-customizado"
        )
...
class TestCrossDomainShape:
    ...
    def test_d03_supervisores(self, api_client, db):
        self._assert_shape(api_client.get("/api/v1/institucional/dres/108100/supervisores/"))

    def test_e01_admin_sgp(self, api_client, db):
        self._assert_shape(api_client.get("/api/v1/institucional/escolas/019251/administrador-sgp/"))

    def test_e05_quantidade_alunos(self, api_client, db):
        self._assert_shape(api_client.get("/api/v1/institucional/escolas/019251/alunos/quantidade/"))

    def test_e07_professores_ano(self, api_client, db):
        self._assert_shape(api_client.get("/api/v1/institucional/escolas/019251/professores/2024/"))

    def test_e08_professores(self, api_client, db):
        self._assert_shape(api_client.get("/api/v1/institucional/escolas/019251/professores/"))

    def test_e09_modalidades(self, api_client, db):
        self._assert_shape(api_client.get("/api/v1/institucional/escolas/modalidades_ensino/"))

    def test_e12_salas(self, api_client, db):
        self._assert_shape(
            api_client.get("/api/v1/institucional/escolas/019251/salas/SALA/anos_letivos/2024/")
        )

    def test_e13_funcionarios(self, api_client, db):
        self._assert_shape(api_client.get("/api/v1/institucional/escolas/019251/funcionarios/"))

    def test_e18_turmas(self, api_client, db):
        self._assert_shape(
            api_client.get("/api/v1/institucional/escolas/019251/turmas/anos_letivos/2024/")
        )
    ...
    def test_t01_sincronizacao_turma(self, api_client, db):
        self._assert_shape(
            api_client.get(
                "/api/v1/institucional/ues/019251/turmas/ABC/sincronizacoes-institucionais/"
            )
        )
