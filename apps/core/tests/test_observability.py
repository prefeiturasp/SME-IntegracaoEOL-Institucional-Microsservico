...
def test_preserva_request_id_recebido() -> None:
    ...
    request = RequestFactory().get(
        ...
        headers={"X-Request-ID": "request-institucional-123"},
    )

    response = _process_request(request)

    assert response["X-Request-ID"] == "request-institucional-123"
    assert request.request_id == "request-institucional-123"  # type: ignore[attr-defined]
...
def test_propaga_request_id_em_diferentes_metodos_http() -> None:
    ...
    for method in ["post", "put", "patch", "delete"]:
        ...
        request = request_func(
            "/api/v1/institucional/dres/", headers=headers, data={}
        )
...
def test_request_id_unico_para_cada_requisicao() -> None:
    ...
    for _ in range(10):
        request = factory.get("/api/v1/institucional/dres/")
        response = _process_request(request)
