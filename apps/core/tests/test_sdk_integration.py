...
def test_middleware_adiciona_request_id_ao_request_object() -> None:
    ...
    request = RequestFactory().get("/api/v1/institucional/dres/")
...
def test_middleware_nao_interfere_em_resposta_json() -> None:
    ...
    request = RequestFactory().get("/api/v1/institucional/dres/108100/")
    ...
    data = json.loads(response.content)
    assert data["codigo_dre"] == "108100"
    assert data["nome"] == "DRE Teste"
    assert data["escolas"] == [1, 2, 3]
...
def test_request_id_header_case_insensitive() -> None:
    ...
    for headers in headers_variants:
        request = factory.get("/api/v1/institucional/dres/", headers=headers)
        response = _process_request(request)
...
def test_middleware_nao_modifica_request_path() -> None:
    ...
    original_path = "/api/v1/institucional/dres/108100/escolas/"
...
def test_middleware_preserva_query_parameters() -> None:
    ...
    request = RequestFactory().get(
        "/api/v1/institucional/escolas/?tipo_escola=1&dre=BT"
    )
